...
// Function to request a generated, temporary Plex oauth link with a unique locally generated UUID4 to determine who made login request both for plex and owner
func InitAuth() (string, int, string) {
	...
	// We use our locally generated uuid4 as well as the returned plex code to create the url to authenticate the user
...
func RequestAuthToken(respID string, clientID string) (string, models.User, error) {
	...
	if !ok {
		fmt.Println("Did not receive 'authToken'")
	}
	...
	go utils.FetchAndParseXML(url1, &user, &wg, errCh)
	go utils.FetchAndParseXML(url2, &mediaContainer, &wg, errCh)
	...
	for err := range errCh {
		fmt.Printf("Error: %v\n", err)
		return "", user, fmt.Errorf("Internal Error: %v\n", err)
	}
